Leafman::Command.new "skel-shoes", "<project-name>", "generate a Shoes skeleton for <project-name>, then set the 'type' accordingly" do |pname|
    ...
    puts "\e[1mappend a skeleton for\e[0m #{pname.rubyize} < Shoes \e[1mto\e[0m #{File.join(File.expand_path(Leafman::PROJECT_DIR), pname, "main.shoes.rb")}"
    File.open(File.join(File.expand_path(Leafman::PROJECT_DIR), pname, "main.shoes.rb"), 'a') do |f|
