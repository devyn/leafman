Leafman::Command.new "show", "<project-name>", "show everything known about the project called <project-name>" do |pname|
    ...
    when 'git'
        ...
        Dir.chdir(p.dir) { puts "...\tat revision \e[1m\e[33m#{`git rev-parse --short HEAD`.chomp}\e[0m." }
    when 'svn'
        puts "...\tuses \e[34m\e[1mSubversion\e[0m#{", syncs" if p['do_update']}."
        Dir.chdir(p.dir) { puts "...\tat revision \e[1m\e[33m#{`svnversion`.chomp}\e[0m." }
    when 'bzr'
        puts "...\tuses \e[33m\e[1mBazaar\e[0m#{", pushes" if p['do_push']}#{", syncs" if p['do_update']}."
        Dir.chdir(p.dir) { puts "...\tat revision \e[1m\e[33m#{`bzr log -r -1`.scan(/^revno: (\d+)$/).flatten.first}\e[0m." }
    when 'hg'
        puts "...\tuses \e[36m\e[1mMercurial\e[0m#{", pushes" if p['do_push']}#{", syncs" if p['do_pull']}."
        Dir.chdir(p.dir) { puts "...\tat revision \e[1m\e[33m#{`hg identify`.chomp}\e[0m." }
    when 'darcs'
