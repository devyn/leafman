Leafman::Command.new "sync", "", "syncs all enabled projects with the server" do
    ...
    Leafman::Projects.each do |p|
        if (p['scm'] == 'git' and p['fetch'])
            puts "\e[1msync-git:\e[0m \e[32m#{p['name']}\e[0m"
