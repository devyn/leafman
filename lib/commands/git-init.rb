Leafman::Command.new "git-init", "<project-name>", "initialize <project-name> as a Git repository" do |pname|
    include Leafman::Mixin
    puts "\e[1mgit-init:\e[0m #{pname}"
    p = Leafman::Projects.find(pname)
    warn("\e[31m\e[1mproject not found.\e[0m")||true&&next unless p
    puts "\e[1mchdir\e[0m #{File.join(File.expand_path(Leafman::PROJECT_DIR), pname)}"
    Dir.chdir(File.join(File.expand_path(Leafman::PROJECT_DIR), pname)) do
        puts "\e[1mgit init\e[0m"
        system('git', 'init') or (warn("\e[31mfailed to initialize.\e[0m")||true&&next)
    end
    puts "\e[1mset SCM to GIT\e[0m"
    p['scm'] = 'git'
    puts "\e[32m\e[1mdone!\e[0m"
end
