...
Leafman::Command.new 'github-auto-fork', '<user>/<repo>', 'automatic github repo forking', 'official addons' do |user_and_repo|
  ...
  login_form.submit
  puts "\e[1mfetching repo page\e[0m"
  repo_page = agent.get '/#{user_and_repo}'
  puts "\e[1mforking repo\e[0m"
  repo_page.links.select{|l|l.text=~/^fork$/i}.first.click
  puts "waiting 30 seconds to be sure that repo is forked..."
  sleep 30
  puts "\e[1mcloning fork\e[0m"
  system 'git', 'clone', "git://github.com/#{me}/#{user_and_repo.split('/').last}", File.join(File.expand_path(Leafman::PROJECT_DIR), user_and_repo.split('/').last)
  ...
  Leafman::Projects.add user_and_repo.split('/').last, 'scm' => 'git', 'type' => nil, 'fetch' => 'forked_from'
